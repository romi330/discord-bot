...
class FeedbackModal(ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        channel = (
            ...
            else interaction.client.get_channel(LOG_CHANNEL)
        )
        if channel:
            ...
            embed.add_field(
                name="Category", value=self.category_field.value, inline=False
            )
            embed.add_field(
                name="Details", value=self.details_field.value, inline=False
            )
            embed.set_author(
