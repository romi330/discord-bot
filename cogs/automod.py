...
class AutoMod(commands.Cog):
        ...
        for guild_id in list(self.user_messages.keys()):
            for user_id in list(self.user_messages[guild_id].keys()):
                self.user_messages[guild_id][user_id] = [
                    msg
                    for msg in self.user_messages[guild_id][user_id]
                    if (
                        datetime.datetime.now(datetime.timezone.utc) - msg["timestamp"]
                    ).total_seconds()
                    < 3600
                ]
        ...
        for guild_id in list(self.recent_messages.keys()):
            for channel_id in list(self.recent_messages[guild_id].keys()):
                self.recent_messages[guild_id][channel_id] = [
                    msg
                    for msg in self.recent_messages[guild_id][channel_id]
                    if (
                        datetime.datetime.now(datetime.timezone.utc) - msg["timestamp"]
                    ).total_seconds()
                    < 3600
    ...
    async def on_message(self, message: discord.Message):
        ...
        if rules.get("spam_detection", False):
            if await self.is_spam(
                message, thresholds["spam_messages"], thresholds["spam_seconds"]
            ):
        ...
        if rules.get("flood_control", False):
            if await self.is_flood(
                message, thresholds["flood_messages"], thresholds["flood_seconds"]
            ):
    ...
    async def is_spam(
        self, message: discord.Message, message_limit: int = 5, time_window: int = 10
    ):
        ...
        self.user_messages[guild_id][user_id].append(
            {"content": message.content, "timestamp": current_time}
        )
    ...
    async def is_flood(
        self, message: discord.Message, message_limit: int = 5, time_window: int = 5
    ):
        guild_id = message.guild.id
        ...
        self.recent_messages[guild_id][channel_id].append(
            {"user": user_id, "timestamp": current_time}
        )
...
class AutoModMainMenu(ui.View):
    ...
    async def feature_settings(self, interaction: discord.Interaction, _: ui.Button):
        view = FeatureSettingsView(self.cog, self.guild_id, self.rules)
        await interaction.response.edit_message(
            content="🛡️ **AutoMod Feature Settings**\nToggle features on/off:", view=view
        )

    @ui.button(label="Word Filters", style=discord.ButtonStyle.primary, emoji="🔤")
    async def word_filters(self, interaction: discord.Interaction, _: ui.Button):
        view = WordFiltersView(self.cog, self.guild_id, self.rules)
        await interaction.response.edit_message(
            content="🔤 **AutoMod Word Filters**\nManage blocked words:", view=view
        )

    @ui.button(
        label="Threshold Settings", style=discord.ButtonStyle.primary, emoji="📊"
    )
    async def threshold_settings(self, interaction: discord.Interaction, _: ui.Button):
    ...
    async def view_status(self, interaction: discord.Interaction, _: ui.Button):
        features = {
    ...
    async def close_menu(self, interaction: discord.Interaction, _: ui.Button):
        await interaction.response.edit_message(
            content="AutoMod settings closed.", view=None
        )
...
class FeatureSettingsView(ui.View):
    ...
    def update_button_states(self):
        button_configs = [
            ...
            (
                self.toggle_emoji_spam_detection,
                "emoji_spam_detection",
        ...
        for button, rule_key, label in button_configs:
            ...
            button.style = (
                discord.ButtonStyle.green if current_value else discord.ButtonStyle.red
            )
    ...
    async def toggle_spam_detection(
        self, interaction: discord.Interaction, button: ui.Button
    ):
    ...
    async def toggle_profanity_filter(
        ...
        await self.toggle_rule(
            interaction, "profanity_filter", "Profanity Filter", button
        )

    @ui.button(label="Emoji Spam: OFF", style=discord.ButtonStyle.red)
    async def toggle_emoji_spam_detection(
        self, interaction: discord.Interaction, button: ui.Button
    ):
        await self.toggle_rule(
            interaction, "emoji_spam_detection", "Emoji Spam", button
        )

    @ui.button(label="Flood Control: OFF", style=discord.ButtonStyle.red)
    async def toggle_flood_control(
        self, interaction: discord.Interaction, button: ui.Button
    ):
...
class WordFiltersView(ui.View):
    ...
    async def view_blocked_words(self, interaction: discord.Interaction, _: ui.Button):
        blocked_words = self.rules.get("blocked_words_list", [])
        if not blocked_words:
            await interaction.response.send_message(
                "No blocked words have been set.", ephemeral=True
            )
            return
    ...
    async def add_blocked_word(self, interaction: discord.Interaction, _: ui.Button):
        modal = BlockedWordModal(
    ...
    async def remove_blocked_word(self, interaction: discord.Interaction, _: ui.Button):
        modal = BlockedWordModal(
...
class BlockedWordModal(ui.Modal):
    word_input = ui.TextInput(
        label="Enter word", placeholder="Enter a word to block or remove"
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not word:
            await interaction.response.send_message(
                "Please enter a valid word.", ephemeral=True
            )
            return
        ...
        if self.is_remove:
            if word in self.rules["blocked_words_list"]:
                ...
                await interaction.response.send_message(
                    f"`{word}` is not in the blocked words list.", ephemeral=True
                )
        else:
            if word not in self.rules["blocked_words_list"]:
                ...
                await interaction.response.send_message(
                    f"`{word}` is already in the blocked words list.", ephemeral=True
                )
...
class ThresholdSettingsView(ui.View):
    ...
    async def spam_settings(self, interaction: discord.Interaction, _: ui.Button):
    ...
    async def flood_settings(self, interaction: discord.Interaction, _: ui.Button):
        modal = ThresholdModal(
    ...
    async def emoji_settings(self, interaction: discord.Interaction, _: ui.Button):
...
class ThresholdModal(ui.Modal):
    def __init__(self, title, cog, guild_id, rules, threshold_type):
        ...
        if "thresholds" not in self.rules:
            ...
            self.cog.rules.setdefault(self.guild_id, {})[
                "thresholds"
            ...
            for key, value in default_thresholds.items():
                if key not in self.rules["thresholds"]:
                    ...
                    self.cog.rules.setdefault(self.guild_id, {}).setdefault(
                        "thresholds", {}
                    )[key] = value
                    self.cog.save_rules()
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            if messages < 1 or messages > 20:
                await interaction.response.send_message(
                    "Message count must be between 1 and 20.", ephemeral=True
                )
                return

            if seconds < 1 or seconds > 300:
                await interaction.response.send_message(
                    "Time window must be between 1 and 300 seconds.", ephemeral=True
                )
                return
            ...
            self.cog.rules[self.guild_id]["thresholds"][
                f"{self.threshold_type}_messages"
            ] = messages
            self.cog.rules[self.guild_id]["thresholds"][
                f"{self.threshold_type}_seconds"
            ] = seconds
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            if limit < 1 or limit > 50:
                await interaction.response.send_message(
                    "Emoji limit must be between 1 and 50.", ephemeral=True
                )
                return
